...
- **Decision policies** (`jev decide`, `jev batch`, and the plugin's opt-in `/jev gate shadow` and `/jev decide_tools on`): only the state fields the policy names, each redacted and capped at 1,500 characters. A state that looks like it holds a secret is not sent. The `facts` your code hands in are never sent, and a decision a pre-rule makes from them sends nothing at all. The command gate sends the tool name, the command with shell comments stripped (up to 2,000 characters), the pattern keys your host already flagged and the *kind* of folder it runs in (home, repo, scratch…), never the path. It never sends the agent's own description of the command. Your own approval policy text (`approvals.smart_policy`), when set, goes in the question as trusted instructions.
